...
test('resolves the Pages base path before checks and build', () => {
  ...
  assert.match(workflow, /Run checks[\s\S]*GITHUB_PAGES_BASE_PATH:\s*\$\{\{ steps\.pages\.outputs\.base_path \}\}/);
  assert.match(workflow, /Build static site[\s\S]*GITHUB_PAGES_BASE_PATH:\s*\$\{\{ steps\.pages\.outputs\.base_path \}\}/);
